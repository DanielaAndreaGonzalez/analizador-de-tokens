...
def tokenizar(codigo_fuente):
    ...
    Además, incluye la posición de inicio de cada token.
    Devuelve una lista de tuplas, cada una representando un token con su tipo, valor y posición de inicio.
    """
    tokens = []
    i = 0
    longitud = len(codigo_fuente)
    while i < longitud:
        inicio_token = i  # Guardar la posición de inicio del token actual
        ...
        if codigo_fuente[i] == '"':
            ...
            tokens.append((token[0], token[1], inicio_token))
            continue
        ...
        if codigo_fuente[i] in '+-*/=;[],{}()':
            ...
            if i + 1 < longitud and codigo_fuente[i+1] == '=' and codigo_fuente[i] in '+-*/<>=!':
                op += '='
                i += 1
            tokens.append(('OPERADOR', op, inicio_token))
            i += 1
            continue
...
def mostrar_tokens(tokens):
    """
    Muestra los tokens en el área de texto de la interfaz gráfica, incluyendo su tipo, valor y posición.
    """
    txt_tokens.delete('1.0', tk.END)  # Limpiar el área de texto
    for tipo, valor, posicion in tokens:
        txt_tokens.insert(tk.END, f"{tipo} ('{valor}', Posición: {posicion})\n")
